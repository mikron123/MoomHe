...
const ColorApplicationDialog = ({ color, onClose, onApply }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[80] flex items-center justify-center p-4 animate-fade-in" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="bg-surface border border-white/10 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl">
        <div className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
          <div className="flex items-center gap-3">
            ...
            ></div>
            <h3 className="text-lg font-semibold text-white">{t('changeColor')}</h3>
          </div>
        ...
        <div className="p-6 space-y-6">
          <div className="space-y-3">
            <label className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 transition-all">
              ...
              <div className="flex-1">
                <span className="font-medium text-white block">{t('allWalls')}</span>
                <span className="text-xs text-gray-400">{t('paintAllWallsInRoom')}</span>
              </div>
            </label>
            ...
            <label className="flex items-center gap-3 p-3 rounded-xl bg-white/5 border border-white/10 cursor-pointer hover:bg-white/10 transition-all">
              ...
              />
              <div className="flex-1">
                <span className="font-medium text-white block">{t('specificObject')}</span>
                <span className="text-xs text-gray-400">{t('chooseWhatToPaint')}</span>
              </div>
            </label>
          </div>
          ...
          {mode === 'custom' && (
            <div className="animate-slide-up">
              <label className="block text-sm text-gray-400 mb-2">{t('whatToPaint')}</label>
              <input
                type="text"
                value={customTarget}
                onChange={(e) => setCustomTarget(e.target.value)}
                placeholder={t('paintExamplePlaceholder')}
          ...
          <div className={`pt-2 flex ${isRTL ? 'justify-start' : 'justify-end'} gap-3`}>
            <button 
              onClick={onClose}
